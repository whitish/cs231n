...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    num_train = scores.shape[0]  # number of observations in output
    ...
    data_loss = np.sum(-1*np.log(correct_probs))
    reg_loss = reg * np.sum(W1*W1) + reg * np.sum(W2*W2)

    loss = data_loss / num_train + reg_loss
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    P = probs - ind  # probs matrix
    dL_dh = np.dot(P, W2.T)  # dL/d_hidden
    dL_dh[hidden_layer <= 0] = 0

    dL_dW2 = np.dot(hidden_layer.T, P)
    dL_dW1 = np.dot(X.T, dL_dh)
    dL_db2 = P
    dL_db1 = dL_dh

    grads['W2'] = dL_dW2 / num_train + 2*reg*W2
    grads['W1'] = dL_dW1 / num_train + 2*reg*W1
    grads['b2'] = np.sum(dL_db2 / num_train, axis=0, keepdims=False)
    grads['b1'] = np.sum(dL_db1 / num_train, axis=0, keepdims=False)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
